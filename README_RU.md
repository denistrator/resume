...
Необходимо [установить `Node.js`](http://nodejs.org/) версии >=20
...
**[TARS-CLI](https://github.com/tars/tars-cli) — предпочтительный способ работы с TARS. [Команда инициализации доступна в TARS-CLI](https://github.com/tars/tars-cli/blob/master/docs/ru/commands.md#tars-init).**
...
**[TARS-CLI](https://github.com/tars/tars-cli) — предпочтительный способ работы с TARS.** Все команды ниже используют `tars` (из TARS-CLI). Без CLI используйте `npx gulp` как запасной вариант.

`tars init` — Инициализирует проект с заданными опциями в tars-config. Создает файловую структуру.

`tars` или `tars build` — делает сборку проекта. Подключаются не минимизированные файлы. Тип сборки зависит от переданных ключей вместе с этой командой. Доступные ключи:
...
`tars dev` — инициализация сборщика в режиме разработки. Создается dev-версия проекта, без всех минификаций. Также запускает вотчеры за файлами проекта. Доступные ключи:
...
`tars build-dev` — генерация dev-версии проекта без вотчеров.
